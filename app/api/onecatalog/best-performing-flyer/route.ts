...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await oneCatalogJsonService.fetchBestPerformingFlyer({ customerId });
    return NextResponse.json(result);
  } catch (error) {
    if (error instanceof DataAccessError) {
      return NextResponse.json({ error: error.message }, { status: 500 });
    }
    return NextResponse.json({ error: "Failed to fetch best performing flyer" }, { status: 500 });
  }
}
